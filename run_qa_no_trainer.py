...
def parse_args():
    ...
    parser.add_argument(
        "--validation_file", type=str, default='/home/yiz/Desktop/ADLHW1/qa_valid_dataset.csv', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
    ...
    parser.add_argument("--output_dir", type=str, default='/home/yiz/Desktop/ADLHW1/output_qa/hfl/chinese-lert-base', help="Where to store the final model.")
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            answers = json.loads(examples[answer_column_name][sample_index])  # 解析 JSON 字符串
            # print(answers)
    ...
    # 初始化列表来存储每个epoch的loss和exact_match值
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        model.train()
        total_loss = 0  # 用于记录该 epoch 的总损失值
        num_batches = 0  # 用于统计 batch 数量
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                outputs = model(**batch)
                loss = outputs.loss
                total_loss += loss.detach().float().item()  # 记录损失
                num_batches += 1
        ...
        loss_values_per_epoch.append(avg_loss)
        logger.info(f"Epoch {epoch}: Average Loss = {avg_loss}")

        # 计算每个 epoch 的 evaluation exact_match
        model.eval()
        all_start_logits = []
        all_end_logits = []
        ...
        max_len = max([x.shape[1] for x in all_start_logits])  # 获取最大长度
        ...
        # 记录每个epoch的exact_match
        exact_match = eval_metric['exact_match']
        exact_match_values.append(exact_match)
        logger.info(f"Epoch {epoch}: exact_match = {exact_match}, eval_metric = {eval_metric}")
    ...
    # 保存 Loss 图像
    loss_plot_file = os.path.join(args.output_dir, "loss_plot.png")
    plt.savefig(loss_plot_file)
    # plt.show()

    logger.info(f"Loss plot saved to {loss_plot_file}")

    # 绘制 Exact Match 曲线图
    plt.figure()
    plt.plot(range(1, len(exact_match_values) + 1), exact_match_values, marker='o')
    plt.xlabel('Epoch')
    ...
    # 保存 Exact Match 图像
    exact_match_plot_file = os.path.join(args.output_dir, "exact_match_plot.png")
    plt.savefig(exact_match_plot_file)
    # plt.show()

    logger.info(f"Exact Match plot saved to {exact_match_plot_file}")
